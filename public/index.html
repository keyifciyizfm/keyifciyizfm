...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #emoji-panel { display: none; position: absolute; bottom: 60px; left: 12px; background: #333; border: 2px solid #555; padding: 12px; display: flex; flex-wrap: wrap; gap: 12px; width: 280px; border-radius: 5px; box-shadow: 0 -5px 20px #000; z-index: 50; }
...
<div id="login-overlay">
    <div id="login-box">
        ...
        <input type="text" id="nick-input" placeholder="Nickiniz..." maxlength="15" oninput="checkAdmin(this.value)" style="padding:10px; margin-bottom:10px; display:block; width:100%;">
        <div id="pass-area" style="display:none;">
            <input type="password" id="pass-input" placeholder="Yönetici Şifresi..." style="padding:10px; margin-bottom:10px; display:block; width:100%;">
        </div>
...
<script>
    ...
    function checkAdmin(v) { 
        document.getElementById('pass-area').style.display = (v === "Keyifciyiz_Fm") ? "block" : "none"; 
    }

    function joinChat() {
        var nick = document.getElementById('nick-input').value;
        var pass = document.getElementById('pass-input').value; // Şifre eklendi
        if(nick.trim() != "") {
            socket.emit('join', { nick: nick, password: pass });
        }
    }

    socket.on('auth error', function(m) { alert(m); });
    socket.on('login success', function() { document.getElementById('login-overlay').style.display = 'none'; });
    ...
    function sendMessage(e) {
        ...
        if (input.value) {
            socket.emit('chat message', { text: input.value, color: selectedColor, style: styles });
            input.value = '';
    ...
    socket.on('chat message', function(data) {
        ...
        if(data.system) {
            ...
            let s = data.style || {};
            div.innerHTML = `<span style="font-weight:bold; color:${data.color}">${data.user}:</span> 
                <span style="color:${data.color}; font-weight:${s.bold?'bold':'normal'}; font-style:${s.italic?'italic':'normal'}; text-decoration:${s.underline?'underline':'none'}; word-wrap: break-word; line-height:1.4;">${data.text}</span>`;
        }
    ...
    socket.on('user list', function(userObjects) {
        ...
        userObjects.forEach(function(u) {
            ...
            // İkonu koruduk, yanına sadece DJ ise etiket ekledik
            let djTag = (u.role === 'DJ') ? ' <small style="font-weight:bold; font-style:italic; border-bottom:1px solid">Dj</small>' : '';
            item.innerHTML = '👤 ' + u.nick + djTag;
