...
// Ses verisi akışı için buffer kapasitesi artırıldı
    ...
    socket.on('stream-status', (status) => {
        if (socket.role === 'Yönetici') {
            io.emit('chat message', { 
                user: "SİSTEM", 
                text: status ? "🔴 Canlı yayın başladı!" : "⚪ Yayın durduruldu.", 
                color: status ? "#e74c3c" : "#95a5a6" 
            });
        }
    });
...
server.listen(process.env.PORT || 3000, () => console.log("Radyo 3000 portunda aktif!"));
